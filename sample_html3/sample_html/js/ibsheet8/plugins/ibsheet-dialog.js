...
Fn.showPivotDialog = function (width, height, name, showType, pivotParams, useStorage) {
  ...
  dialogOpt.Body = "<div class='" + themePrefix + classDlg + "' style='width:" + width + "px;height:" + height + "px;' onmousedown='return false' onkeyup='" + this.id + ".closePivotDialog(event)' onmouseup='" + this.id + ".PivotDragMouseUp(event)' onmousemove='" + this.id + ".PivotDragMouseMove(event)'>" +
    ...
    "<div id='" + name + "_PivotRow' class='" + name + "_PivotStandards pivotDialogDropzoneRowCol' style='height:calc(100% - 45px);padding:7px; overflow-y: auto;' onmouseup='" + this.id + ".PivotDragSetItem(this,event,1)'>" +
    ...
    "<div id='" + name + "_PivotCol' class='" + name + "_PivotStandards pivotDialogDropzoneRowCol' style='height:calc(100% - 46px);padding:7px; overflow-y: auto;' onmouseup='" + this.id + ".PivotDragSetItem(this,event,1)'>" +
    ...
    "<div id='" + name + "_PivotData' class='" + name + "_PivotStandards pivotDialogDropzoneVal' style='height:calc(100% - 46px);padding:7px; overflow-y: auto;' onmouseup='" + this.id + ".PivotDragSetItem(this,event,2)'>" +
...
Fn.findPivotColumn = function () {
  ...
  var cols = this.getCols().filter(function (col) {
    return col !== "SEQ" && !this.Cols[col]["NoPivotDialog"];
  }.bind(this));
...
Fn.producePivotColumn = function () {
  ...
  if (cols.common && cols.common.length > 0) {
    for (var i = 0; i < cols.common.length; i++) {
      var strCols = "<span onmousedown='" + this.id + ".PivotItemClick(this,event);' ontouchstart='" + this.id + ".PivotItemClick(this,event);'><b class='box colText' colName='" + cols.common[i].Name + "'>" + cols.common[i].Value + "</b></span>";
  ...
  if (cols.number && cols.number.length > 0) {
    for (var i = 0; i < cols.number.length; i++) {
      var strCols = "<span onmousedown='" + this.id + ".PivotItemClick(this,event);' ontouchstart='" + this.id + ".PivotItemClick(this,event);'><b class='box colValue' colName='" + cols.number[i].Name + "'>" + cols.number[i].Value + "</b></span>";
      if (saveInfo) {
